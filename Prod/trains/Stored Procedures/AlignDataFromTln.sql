    ...
    BEGIN
        BEGIN TRY
            ...
            INSERT INTO trains.[TrainStations] ( LegacyStationId ,
                        ...
                        WHERE  [IDStazione] IN (   SELECT [IDStazione]
                                                   FROM   [tln].[vol_StazioneSbme]
                                                   EXCEPT
                                                   SELECT LegacyStationId
                                                   FROM   trains.TrainStations
                                                   WHERE  LegacyStationId IS NOT NULL );

            PRINT '1. Inserimento nuove stazioni';

            UPDATE trains.TrainStations
            SET    Valid = 0
            WHERE  LegacyStationId IN (   SELECT LegacyStationId
                                          FROM   trains.TrainStations
                                          WHERE  LegacyStationId IS NOT NULL
                                          EXCEPT
                                          SELECT [IDStazione]
        ...
        BEGIN TRY
            ...
            INSERT INTO trains.[TrainPaths] ( LegacyPathId ,
                        ...
                        WHERE  [IdPercorso] IN (   SELECT IdPercorso
            ...
            SET    Valid = 0
            WHERE  LegacyPathId IN (   SELECT LegacyPathId
                                       FROM   trains.[TrainPaths]
                                       WHERE  LegacyPathId IS NOT NULL
                                       EXCEPT
                                       SELECT [IdPercorso]
                                       FROM   [tln].[vol_Percorso] );
        ...
        BEGIN TRY
            ...
            INSERT INTO [dbo].[CardProfiles] ( [CardProfileId] ,
                        ...
                        WHERE  [CodiceRichiestaProfilo] IN (   SELECT [CodiceRichiestaProfilo]
                                                               FROM   [tln].[vol_CodiceRichiestaProfilo]
                                                               EXCEPT
                                                               SELECT [CardProfileId]
                                                               FROM   [CardProfiles] );
            ...
            SET    Valid = 0
            WHERE  [CardProfileId] IN (   SELECT [CardProfileId]
                                          FROM   [CardProfiles]
                                          EXCEPT
                                          SELECT [CodiceRichiestaProfilo]
                                          FROM   [tln].[vol_CodiceRichiestaProfilo] );
